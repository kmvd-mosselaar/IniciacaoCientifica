...
Forest. A pesquisa se baseia em dados de uma indústria petroquímica, incluindo variáveis como correntes, pressões e temperaturas
em diferentes pontos da máquina. A metodologia incluiu o préprocessamento dos dados, a construção de janelas temporais, a
otimização de hiperparâmetros e a implementação de diferentes
variações dos modelos. Os resultados iniciais indicaram problemas de sobreajuste (overfitting), abordados por meio de filtragem
dos dados, normalização separada de treino e teste, atribuição
de pesos para classes raras, além da aplicação de técnicas como
dropout e parada antecipada. A avaliação dos modelos finais
